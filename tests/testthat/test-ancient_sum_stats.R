test_that("ancient_sum_stats works",{
  ...
  pseudo_G = pseudo_hap(dmg_G, seed = age_seed)
  ...
  output = suppressWarnings(
    ancient_sum_stats(sim = sim,split_type="mut",nwins = nwins, ID=id,
                      ascertain_indices = asc_index, missing_rate = missing_rate,trans_prop = trans_prop
                      ,seed = age_seed,impute_method = impute_method,
                      denoise_method = denoise_method, max_clus = mclus)
  )
  ...
  win_clus_vec = suppressWarnings(
      lapply(hap_win_list, function(G){clus_hap(G, max_clus = round( nrow(G)*mclus) ) })
  )
  ...
  #test 5, denoise_method = "majority_flip"
  ...
  #test 5, denoise_method = "fixed_cluster" ----
  ...
  output = suppressWarnings(
    ancient_sum_stats(sim = sim,split_type="mut",nwins = nwins, ID=id,
                      ascertain_indices = asc_index, missing_rate = missing_rate,trans_prop = trans_prop
                      ,seed = age_seed,impute_method = impute_method,
                      denoise_method = denoise_method, fixed_clus = max_clus)
  )
  ...
  #if haplotype stats disagree, check the max_clus argument for the clustering
  
  #test 6, denoise_method = "majority_flip" ----
